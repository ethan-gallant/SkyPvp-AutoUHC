...
public class TeamSelectorMenu extends Menu {
	...
	public void show() {
		...
		for(Team team : UHCSystem.getTeams()) {
			...
			players = players.replaceAll("\\{maxMembers\\}", String.valueOf(Globals.MAX_MEMBERS_PER_TEAM));
			lore.add(msgs.color(players));
			
			if(team.getMembers().size() < Globals.MAX_MEMBERS_PER_TEAM) {
		...
		viewer.getBukkitPlayer().updateInventory();
	}
	...
	public void clickPerformed(InventoryClickEvent evt) {
		...
		if(team != null && team.getMembers().size() < Globals.MAX_MEMBERS_PER_TEAM) {
